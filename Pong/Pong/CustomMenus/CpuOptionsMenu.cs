...
namespace Pong.CustomMenus
{
    class CpuOptionsMenu : Menu
    {
        ...
        // Controles del menú
        ElementSelectorControl<string> dificultyControl = new ElementSelectorControl<string>("Dificultad: ");
        ...
        public CpuOptionsMenu(string name)
            : base(name)
        {
            // Asignación de Handlers para los eventos de los controles
            this.dificultyControl.ValueChangued += new ElementSelectorControl<string>.ValueChangedHandler(dificultyControl_ValueChangued);
            ...
            // Se agregan las distintas dificultades al control
            dificultyControl.AddElement("Facil");
            dificultyControl.AddElement("Medio");
            dificultyControl.AddElement("Dificil");

            // Se asignan por default los valores a los controles
            dificultyControl.Value = "Medio";
            maxScoreControl.MinValue = 1;
            maxScoreControl.MaxValue = 25;
            maxScoreControl.Value = 10;

            // Se agregan los controles al menú
            AddControl(dificultyControl);
        ...
        /// <summary>
        /// Evento que ocurre al cambiar el valor de "Dificultad: "
        /// </summary>
        /// <param name="control">Control que produce el evento</param>
        void dificultyControl_ValueChangued(Control control)
        {
            switch (dificultyControl.Value)
            {
                case "Facil":
                    Global.Dificulty = 0;
                    break;
